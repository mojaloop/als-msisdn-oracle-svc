...
async function main() {
  ...
  try {
    ...
    while (!allHealthy && retries > 0) {
      ...
      if (retries === 0) {
        throw new Error(
          `Out of retries waiting for service health.\nStill waiting for: ${getServicesForStatus(
            waitingMap,
            'starting'
          )}`
        )
      ...
      console.log(
        `${getServicesForStatus(waitingMap, 'healthy').length} services are healthy. Expected: ${
...
async function updateServiceStatus(waitingMap) {
  ...
  Promise.all(
    startingServices.map(async (serviceName) => {
...
function getServicesForStatus(waitingMap, status) {
  return Object.keys(waitingMap).filter((k) => waitingMap[k] === status)
}
...
async function sleep(timeMs) {
  ...
  return new Promise((resolve, _reject) => setTimeout(() => resolve(), timeMs))
}
